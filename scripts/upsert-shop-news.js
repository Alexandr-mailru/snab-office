...
const news = [
  ...
  },
  {
    ...
    body: [
      ...
      "Возможности художественных пудр Cretacolor. Материал подходит для различных техник рисования: создания широких заливок и фоновых пятен; быстрого выполнения набросков и эскизов; проработки объёма, света и тени; мягкой растушёвки и создания плавных переходов; получения акварельных эффектов при добавлении воды; смешивания со связующими материалами для создания собственных художественных составов.",
...
async function main() {
  ...
  for (const item of news) {
    ...
    console.log("ok", item.slug, "body", item.body.length, "cover", item.coverImage);
  }
}
